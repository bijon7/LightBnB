...
const addUser = function (user) {
  ...
  return pool
    ...
    .then((result) => {
      console.log("user log", result.rows);
      return result.rows[0];
    })
    .catch((err) => {
      console.log(err.message);
...
const getAllReservations = function (guest_id, limit = 10) {
  return pool
    .query(`SELECT * FROM properties WHERE users.id = $1 LIMIT $2;`, [guest_id, limit])
    .then((result) => {
      console.log(result.rows);
      return result.rows;
    })
